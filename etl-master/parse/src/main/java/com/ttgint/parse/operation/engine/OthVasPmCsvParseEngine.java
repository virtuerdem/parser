...
import com.ttgint.parse.operation.handler.OthVasPmCsvParseHandler;
...
@Slf4j
@Component("OTH_VAS_PM_PARSE")
public class OthVasPmCsvParseEngine extends ParseBaseEngine {

    public OthVasPmCsvParseEngine(ApplicationContext applicationContext) {
        super(applicationContext);
    }

    @Override
    protected void onEngine() {
        log.info("* OthVasPmCsvParseEngine onEngine");

        ArrayList<File> files
                = new ArrayList<>(fileLib.readFilesInWalkingPathByPostfix(engineRecord.getRawPath(), ".csv"));
        log.info("* OthVasPmCsvParseEngine onEngine fileSize: {}", files.size());

        ExecutorService executor = Executors.newFixedThreadPool(engineRecord.getOnParseThreadCount());
        files.forEach(file -> {
            try {
                ParseBaseHandler handler = new OthVasPmCsvParseHandler(
                        applicationContext,
                        ParseHandlerRecord.getRecord(engineRecord, file, ProgressType.TEST));
                executor.execute(handler);
            } catch (Exception exception) {
                log.error("! OthVasPmCsvParseEngine handler creation/execution failed for file: {}",
