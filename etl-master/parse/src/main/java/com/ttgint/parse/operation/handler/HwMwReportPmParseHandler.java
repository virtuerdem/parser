...
@Slf4j
public class HwMwReportPmParseHandler extends ParseCsvHandler {
    ...
    public HwMwReportPmParseHandler(ApplicationContext applicationContext,
    ...
    public void preHandler() {
        ...
        // measInfo = everything except the last two underscore-separated parts (_date_HH-mm-ss.csv)
        measInfo = originalName.replace("_" + datePart + "_" + parts[parts.length - 1], "");

        // Supports both yyyy-MM-dd and MM-dd-yyyy date formats in filename
        ...
        headerKeyValue.put("etlApp.constant_fileDate",
                stringDateFormatter(datePart + " " + timeFormatted + "+03:00",
                        dateFormat.replace("HH:mmXXX", "HH:mm:ssXXX"), "yyyy-MM-dd HH:mmZ"));
    ...
    public void lineProgress(Long lineIndex, String[] line) {
        // File format: metadata block at top (lines with a single column),
        // then column header row (first line with more than one column), then data rows.
        if (!headerFound) {
            if (line.length > 1) {
    ...
    private void write() {
        ...
        if (parseMap != null) {
            ...
            log.warn("! HwMwReportPmParseHandler no parse map found for measInfo: {}", measInfo);
        }
    }
}
