...
import com.ttgint.parse.operation.handler.HwRncCmXmlParseHandler;
...
@Slf4j
@Component("HW_RNC_CM_XML_PARSE")
public class HwRncCmXmlParseEngine extends ParseBaseEngine {

    public HwRncCmXmlParseEngine(ApplicationContext applicationContext) {
    ...
    protected void onEngine() {
        ...
        log.info("* HwRncCmXmlParseEngine onEngine activeNodeSize: {}", nodeIds.size());

        ArrayList<File> files
                = new ArrayList<>(fileLib.readFilesInWalkingPathByPostfix(engineRecord.getRawPath(), ".xml"));
        log.info("* HwRncCmXmlParseEngine onEngine fileSize: {}", files.size());

        ExecutorService executor = Executors.newFixedThreadPool(engineRecord.getOnParseThreadCount());
        files.forEach(file -> {
            try {
                ParseBaseHandler handler = new HwRncCmXmlParseHandler(applicationContext,
