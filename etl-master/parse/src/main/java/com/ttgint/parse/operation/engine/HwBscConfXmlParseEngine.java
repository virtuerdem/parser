...
import com.ttgint.parse.operation.handler.HwBscConfXmlParseHandler;
...
@Slf4j
@Component("HW_BSC_CONF_XML_PARSE")
public class HwBscConfXmlParseEngine extends ParseBaseEngine {

    public HwBscConfXmlParseEngine(ApplicationContext applicationContext) {
    ...
    protected void onEngine() {
        ...
        log.info("* HwBscConfXmlParseEngine onEngine activeNodeSize: {}", nodeIds.size());

        ArrayList<File> files
                = new ArrayList<>(fileLib.readFilesInWalkingPathByPostfix(engineRecord.getRawPath(), ".xml"));
        log.info("* HwBscConfXmlParseEngine onEngine fileSize: {}", files.size());

        ExecutorService executor = Executors.newFixedThreadPool(engineRecord.getOnParseThreadCount());
        files.forEach(file -> {
            try {
                ParseBaseHandler handler = new HwBscConfXmlParseHandler(applicationContext,
