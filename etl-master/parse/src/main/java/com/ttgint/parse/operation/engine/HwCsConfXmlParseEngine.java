...
import com.ttgint.parse.operation.handler.HwCsConfXmlParseHandler;
...
@Slf4j
@Component("HW_CS_CONF_XML_PARSE")
public class HwCsConfXmlParseEngine extends ParseBaseEngine {

    public HwCsConfXmlParseEngine(ApplicationContext applicationContext) {
    ...
    protected void onEngine() {
        ...
        log.info("* HwCsConfXmlParseEngine onEngine activeNodeSize: {}", nodeIds.size());

        ArrayList<File> files
                = new ArrayList<>(fileLib.readFilesInWalkingPathByPostfix(engineRecord.getRawPath(), ".xml"));
        log.info("* HwCsConfXmlParseEngine onEngine fileSize: {}", files.size());

        ExecutorService executor = Executors.newFixedThreadPool(engineRecord.getOnParseThreadCount());
        files.forEach(file -> {
            try {
                ParseBaseHandler handler = new HwCsConfXmlParseHandler(applicationContext,
