...
@Slf4j
public abstract class ParseTextHandler extends ParseBaseHandler {

    private String splitter;

    public ParseTextHandler(ApplicationContext applicationContext, ParseHandlerRecord handlerRecord, String splitter) {
    ...
    public void onHandler() {
        ...
        try (FileReader fileReader
            ...
            while ((line = bufferedReader.readLine()) != null) {
                lineProgress(lineIndex, line.split(splitter));
                lineIndex++;
            }
            deleteFile(getHandlerRecord().getProgressType());
        } catch (Exception exception) {
            log.error("text parse error lineNumber: {}", lineIndex, exception);
