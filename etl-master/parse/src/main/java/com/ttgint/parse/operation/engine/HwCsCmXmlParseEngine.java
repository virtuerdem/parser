...
import com.ttgint.parse.operation.handler.HwCsCmXmlParseHandler;
...
@Slf4j
@Component("HW_CS_CM_XML_PARSE")
public class HwCsCmXmlParseEngine extends ParseBaseEngine {

    public HwCsCmXmlParseEngine(ApplicationContext applicationContext) {
    ...
    protected void onEngine() {
        ...
        log.info("* HwCsCmXmlParseEngine onEngine activeNodeSize: {}", nodeIds.size());

        ArrayList<File> files
                = new ArrayList<>(fileLib.readFilesInWalkingPathByPostfix(engineRecord.getRawPath(), ".xml"));
        log.info("* HwCsCmXmlParseEngine onEngine fileSize: {}", files.size());

        ExecutorService executor = Executors.newFixedThreadPool(engineRecord.getOnParseThreadCount());
        files.forEach(file -> {
            try {
                ParseBaseHandler handler = new HwCsCmXmlParseHandler(applicationContext,
