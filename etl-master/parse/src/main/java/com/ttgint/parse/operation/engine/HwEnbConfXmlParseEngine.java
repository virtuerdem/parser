...
import com.ttgint.parse.operation.handler.HwEnbConfXmlParseHandler;
...
@Slf4j
@Component("HW_ENB_CONF_XML_PARSE")
public class HwEnbConfXmlParseEngine extends ParseBaseEngine {

    public HwEnbConfXmlParseEngine(ApplicationContext applicationContext) {
    ...
    protected void onEngine() {
        ...
        log.info("* HwEnbConfXmlParseEngine onEngine activeNodeSize: {}", nodeIds.size());

        ArrayList<File> files
                = new ArrayList<>(fileLib.readFilesInWalkingPathByPostfix(engineRecord.getRawPath(), ".xml"));
        log.info("* HwEnbConfXmlParseEngine onEngine fileSize: {}", files.size());

        ExecutorService executor = Executors.newFixedThreadPool(engineRecord.getOnParseThreadCount());
        files.forEach(file -> {
            try {
                ParseBaseHandler handler = new HwEnbConfXmlParseHandler(applicationContext,
