...
@Slf4j
@Component("HW_GNB_PM_TRANSFER")
public class HwGnbPmTransferEngine extends TransferBaseEngine {

    private final NetworkNodeRepository networkNodeRepository;

    public HwGnbPmTransferEngine(ApplicationContext applicationContext) {
    ...
    protected void onEngine() {
        log.info("* HwGnbPmTransferEngine onTransfer");
        List<String> nodes
        ...
        getConnections()
                .forEach(connection -> {
                    try {
                        executor.execute(
                                new HwGnbPmTransferHandler(
                                        applicationContext,
                                        getTransferHandlerRecord(connection),
                                        nodes)
                        );
                    } catch (Exception exception) {
                        log.error("! HwGnbPmTransferEngine onProcess connectionId:{} error: {}", connection.getId(),
