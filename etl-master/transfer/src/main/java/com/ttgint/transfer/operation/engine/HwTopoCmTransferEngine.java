...
import com.ttgint.transfer.operation.handler.HwTopoCmTransferHandler;
...
@Slf4j
@Component("HW_TOPO_CM_TRANSFER")
public class HwTopoCmTransferEngine extends TransferBaseEngine {

    private final NetworkItemRepository networkItemRepository;

    public HwTopoCmTransferEngine(ApplicationContext applicationContext) {
    ...
    protected void onEngine() {
        log.info("* HwTopoCmTransferEngine onTransfer");
        ...
        getConnections()
                .forEach(connection -> {
                    try {
                        executor.execute(
                                new HwTopoCmTransferHandler(
                                        applicationContext,
                                        getTransferHandlerRecord(connection),
                                        items)
                        );
                    } catch (Exception exception) {
                        log.error("! HwTopoCmTransferEngine onProcess connectionId:{} error: {}",
                                connection.getId(), exception.getMessage());
                    }
                });
