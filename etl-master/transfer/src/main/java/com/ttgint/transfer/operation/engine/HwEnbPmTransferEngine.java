...
import com.ttgint.transfer.operation.handler.HwEnbPmTransferHandler;
...
@Slf4j
@Component("HW_ENB_PM_TRANSFER")
public class HwEnbPmTransferEngine extends TransferBaseEngine {

    private final NetworkNodeRepository networkNodeRepository;

    public HwEnbPmTransferEngine(ApplicationContext applicationContext) {
    ...
    protected void onEngine() {
        log.info("* HwEnbPmTransferEngine onTransfer");
        List<String> nodes
        ...
        getConnections()
                .forEach(connection -> {
                    try {
                        executor.execute(
                                new HwEnbPmTransferHandler(
                                        applicationContext,
                                        getTransferHandlerRecord(connection),
                                        nodes)
                        );
                    } catch (Exception exception) {
                        log.error("! HwEnbPmTransferEngine onProcess connectionId:{} error: {}", connection.getId(),
