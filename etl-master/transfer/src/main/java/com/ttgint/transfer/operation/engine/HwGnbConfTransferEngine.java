...
import com.ttgint.transfer.operation.handler.HwGnbConfTransferHandler;
...
@Slf4j
@Component("HW_GNB_CONF_TRANSFER")
public class HwGnbConfTransferEngine extends TransferBaseEngine {

    private final NetworkNodeRepository networkNodeRepository;

    public HwGnbConfTransferEngine(ApplicationContext applicationContext) {
    ...
    protected void onEngine() {
        log.info("* HwGnbConfTransferEngine onTransfer");
        List<String> nodes
        ...
        getConnections()
                .forEach(connection -> {
                    try {
                        executor.execute(
                                new HwGnbConfTransferHandler(
                                        applicationContext,
                                        getTransferHandlerRecord(connection),
                                        nodes)
                        );
                    } catch (Exception exception) {
                        log.error("! HwGnbConfTransferEngine onProcess connectionId:{} error: {}", connection.getId(),
