...
import com.ttgint.transfer.operation.handler.OthHpeKpiCmTransferHandler;
...
@Slf4j
@Component("OTH_HPE_KPI_CM_TRANSFER")
public class OthHpeKpiCmTransferEngine extends TransferBaseEngine {

    private final NetworkItemRepository networkItemRepository;
    private final NetworkNodeRepository networkNodeRepository;

    public OthHpeKpiCmTransferEngine(ApplicationContext applicationContext) {
    ...
    protected void onEngine() {
        log.info("* HpKpiCmTransferEngine onTransfer");
        List<String> items
        ...
        getConnections()
                .forEach(connection -> {
                    try {
                        executor.execute(
                                new OthHpeKpiCmTransferHandler(
                                        applicationContext,
                        ...
                        log.error("! HpKpiCmTransferEngine onProcess connectionId:{} error: {}", connection.getId(),
