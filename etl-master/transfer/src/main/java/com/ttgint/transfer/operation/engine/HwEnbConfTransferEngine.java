...
import com.ttgint.transfer.operation.handler.HwEnbConfTransferHandler;
...
@Slf4j
@Component("HW_ENB_CONF_TRANSFER")
public class HwEnbConfTransferEngine extends TransferBaseEngine {

    private final NetworkNodeRepository networkNodeRepository;

    public HwEnbConfTransferEngine(ApplicationContext applicationContext) {
    ...
    protected void onEngine() {
        log.info("* HwEnbConfTransferEngine onTransfer");
        List<String> nodes
        ...
        getConnections()
                .forEach(connection -> {
                    try {
                        executor.execute(
                                new HwEnbConfTransferHandler(
                                        applicationContext,
                                        getTransferHandlerRecord(connection),
                                        nodes)
                        );
                    } catch (Exception exception) {
                        log.error("! HwEnbConfTransferEngine onProcess connectionId:{} error: {}", connection.getId(),
