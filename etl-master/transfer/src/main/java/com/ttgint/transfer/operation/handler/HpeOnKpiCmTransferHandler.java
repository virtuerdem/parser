...
@Slf4j
public class HpeOnKpiCmTransferHandler extends TransferBaseHandler {

    private final List<String> items;
    private final List<String> nodes;

    public HpeOnKpiCmTransferHandler(ApplicationContext applicationContext,
    ...
    public void filterFiles() {
        log.info("* HpeOnKpiCmTransferHandler filterFiles and setFileInfos");
        super.filterFiles();
        getRemoteFiles().stream()
                .filter(e -> e.getFileName().startsWith("degapp") && e.getFileName().endsWith(".gz"))
                .filter(e -> {
                    try {
