...
import com.ttgint.transfer.operation.handler.HpeOnKpiCmTransferHandler;
...
@Slf4j
@Component("HPE_ON_KPI_CM_TRANSFER")
public class HpeOnKpiCmTransferEngine extends TransferBaseEngine {

    private final NetworkItemRepository networkItemRepository;
    private final NetworkNodeRepository networkNodeRepository;

    public HpeOnKpiCmTransferEngine(ApplicationContext applicationContext) {
    ...
    protected void onEngine() {
        log.info("* HpeOnKpiCmTransferEngine onTransfer");
        List<String> items
        ...
        getConnections()
                .forEach(connection -> {
                    try {
                        ...
                        log.error("! HpeOnKpiCmTransferEngine onProcess connectionId:{} error: {}", connection.getId(),
    ...
    protected ArrayList<File> getDecompressFiles() {
        return new ArrayList<>(fileLib.readFilesInCurrentPathByContains(engineRecord.getRawPath(), ".gz"));
    }
    ...
    protected OffsetDateTime getDecompressRecordTime(String fileName) {
        return getDecompressRecordTime(
                fileName.split("_", 3)[1].substring(0, fileName.split("_", 3)[1].length() - 2) + " +03:00",
                "yyyyMMddHHmm XXX");
    }
