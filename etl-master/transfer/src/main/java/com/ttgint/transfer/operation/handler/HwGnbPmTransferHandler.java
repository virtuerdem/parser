...
@Slf4j
public class HwGnbPmTransferHandler extends TransferBaseHandler {

    private final List<String> nodes;

    public HwGnbPmTransferHandler(ApplicationContext applicationContext,
    ...
    public void filterFiles() {
        log.info("* HwGnbPmTransferHandler filterFiles and setFileInfos");
        super.filterFiles();
        getRemoteFiles().stream()
                .filter(e -> e.getFileName().startsWith("A") && e.getFileName().contains(".xml")
                        && (e.getFileName().split("_", 2)[1].startsWith("NR")
                        || e.getFileName().split("_", 2)[1].contains("_NR_")
                        || e.getFileName().split("_", 2)[1].contains("_NR.")))
                .filter(e -> {
                    try {
