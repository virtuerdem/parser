...
@Slf4j
public class HpeOnCmTransferHandler extends TransferBaseHandler {
    private final List<String> items;
    private final List<String> nodes;

    public HpeOnCmTransferHandler(ApplicationContext applicationContext,
    ...
    public void filterFiles() {
        log.info("* HpeOnCmTransferHandler filterFiles and setFileInfos");
        super.filterFiles();
        getRemoteFiles().stream()
                .filter(e -> e.getFileName().startsWith("degapp"))
                .filter(e -> {
                    try {
