...
@Slf4j
public class HwGnbCmTransferHandler extends TransferBaseHandler {


    private final List<String> nodes;

    public HwGnbCmTransferHandler(ApplicationContext applicationContext,
    ...
    public void filterFiles() {
        log.info("* HwGnbCmTransferHandler filterFiles and setFileInfos");
        super.filterFiles();
        getRemoteFiles().stream()
                .filter(e -> e.getFileName().startsWith("CMExport_NR") ||
                        (e.getFileName().contains("_NR_")
                                && (e.getFileName().startsWith("CMExport_SR")
                                || e.getFileName().startsWith("CMExport_GL")
                                || e.getFileName().startsWith("CMExport_GU"))))
                .filter(e -> {
                    try {
