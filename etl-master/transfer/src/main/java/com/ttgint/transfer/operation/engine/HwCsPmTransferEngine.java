...
import com.ttgint.transfer.operation.handler.HwCsPmTransferHandler;
...
@Slf4j
@Component("HW_CS_PM_TRANSFER")
public class HwCsPmTransferEngine extends TransferBaseEngine {
    public NetworkNodeRepository networkNodeRepository;

    public HwCsPmTransferEngine(ApplicationContext applicationContext) {
    ...
    protected void onEngine() {
        log.info("* HwCsPmTransferEngine onTransfer");
        List<String> nodes
        ...
        getConnections()
                .forEach(connection -> {
                    try {
                        executor.execute(
                                new HwCsPmTransferHandler(
                                        applicationContext,
                                        getTransferHandlerRecord(connection),
                                        nodes)
                        );
                    } catch (Exception exception) {
                        log.error("! HwCsPmTransferEngine onProcess connectionId:{} error: {}", connection.getId(),
    ...
    protected DecompressRecord getDecompressRecord(File file) {
        return DecompressRecord.getRecord(engineRecord,
                ...
                OffsetDateTime.parse(
                        file.getName().split("A")[1].substring(0, 18),
