...
import com.ttgint.transfer.operation.handler.HwBscCmTransferHandler;
...
@Slf4j
@Component("HW_BSC_CM_TRANSFER")
public class HwBscCmTransferEngine extends TransferBaseEngine {

    private final NetworkNodeRepository networkNodeRepository;

    public HwBscCmTransferEngine(ApplicationContext applicationContext) {
    ...
    protected void onEngine() {
        log.info("* HwBscCmTransferEngine onTransfer");
        List<String> nodes
        ...
        getConnections()
                .forEach(connection -> {
                    try {
                        executor.execute(
                                new HwBscCmTransferHandler(
                                        applicationContext,
                                        getTransferHandlerRecord(connection),
                                        nodes)
                        );
                    } catch (Exception exception) {
                        log.error("! HwBscCmTransferEngine onProcess connectionId:{} error: {}", connection.getId(),
