...
@Slf4j
public class HwEnbPmTransferHandler extends TransferBaseHandler {

    private final List<String> nodes;

    public HwEnbPmTransferHandler(ApplicationContext applicationContext,
    ...
    public void filterFiles() {
        log.info("* HwEnbPmTransferHandler filterFiles and setFileInfos");
        super.filterFiles();
        getRemoteFiles().stream()
                .filter(e -> e.getFileName().startsWith("A") && e.getFileName().contains(".xml")
                        && (e.getFileName().split("_")[1].startsWith("SR")
                        || e.getFileName().split("_")[1].startsWith("GL")
                        || e.getFileName().split("_")[1].startsWith("GU")))
                .filter(e -> {
                    try {
