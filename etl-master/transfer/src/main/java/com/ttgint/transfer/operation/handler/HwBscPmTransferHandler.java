...
@Slf4j
public class HwBscPmTransferHandler extends TransferBaseHandler {

    private final List<String> nodes;

    public HwBscPmTransferHandler(ApplicationContext applicationContext,
    ...
    public void filterFiles() {
        log.info("* HwBscPmTransferHandler filterFiles and setFileInfos");
        super.filterFiles();
        getRemoteFiles().stream()
                .filter(e -> e.getFileName().startsWith("A") && e.getFileName().contains(".xml")
                        && e.getFileName().split("_")[1].startsWith("B"))
                .filter(e -> {
                    try {
