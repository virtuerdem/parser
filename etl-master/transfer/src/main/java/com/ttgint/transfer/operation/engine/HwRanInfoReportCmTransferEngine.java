...
import com.ttgint.transfer.operation.handler.HwRanInfoReportCmTransferHandler;
...
@Slf4j
@Component("HW_RI_REPORT_CM_TRANSFER")
public class HwRanInfoReportCmTransferEngine extends TransferBaseEngine {

    private final NetworkItemRepository networkItemRepository;

    public HwRanInfoReportCmTransferEngine(ApplicationContext applicationContext) {
    ...
    protected void onEngine() {
        log.info("* HwRanInfoReportCmTransferEngine onTransfer");
        List<String> items
        ...
        getConnections()
                .forEach(connection -> {
                    try {
                        executor.execute(
                                new HwRanInfoReportCmTransferHandler(
                                        applicationContext,
                                        getTransferHandlerRecord(connection),
                                        items)
                        );
                    } catch (Exception exception) {
                        log.error("! HwRanInfoReportCmTransferEngine onProcess connectionId:{} error: {}", connection.getId(),
