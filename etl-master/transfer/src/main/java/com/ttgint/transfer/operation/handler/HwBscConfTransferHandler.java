...
@Slf4j
public class HwBscConfTransferHandler extends TransferBaseHandler {

    private final List<String> nodes;

    public HwBscConfTransferHandler(ApplicationContext applicationContext,
    ...
    public void filterFiles() {
        log.info("* HwBscConfTransferHandler filterFiles and setFileInfos");
        super.filterFiles();
        getRemoteFiles().stream()
                .filter(e -> e.getFileName().startsWith("GExport_B"))
                .filter(e -> {
                    try {
