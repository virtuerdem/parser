...
@Slf4j
public class HwGnbConfTransferHandler extends TransferBaseHandler {

    private final List<String> nodes;

    public HwGnbConfTransferHandler(ApplicationContext applicationContext,
    ...
    public void filterFiles() {
        log.info("* HwGnbConfTransferHandler filterFiles and setFileInfos");
        super.filterFiles();
        getRemoteFiles().stream()
                .filter(e -> e.getFileName().startsWith("GExport_NR") ||
                        (e.getFileName().contains("_NR_")
                                && (e.getFileName().startsWith("GExport_SR")
                                || e.getFileName().startsWith("GExport_GL")
                                || e.getFileName().startsWith("GExport_GU"))))
                .filter(e -> {
                    try {
