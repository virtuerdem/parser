...
import com.ttgint.transfer.operation.handler.HwRncCmTransferHandler;
...
@Slf4j
@Component("HW_RNC_CM_TRANSFER")
public class HwRncCmTransferEngine extends TransferBaseEngine {

    private final NetworkNodeRepository networkNodeRepository;

    public HwRncCmTransferEngine(ApplicationContext applicationContext) {
    ...
    protected void onEngine() {
        log.info("* HwRncCmTransferEngine onTransfer");
        List<String> nodes
        ...
        getConnections()
                .forEach(connection -> {
                    try {
                        executor.execute(
                                new HwRncCmTransferHandler(
                                        applicationContext,
                                        getTransferHandlerRecord(connection),
                                        nodes)
                        );
                    } catch (Exception exception) {
                        log.error("! HwRncCmTransferEngine onProcess connectionId:{} error: {}", connection.getId(),
