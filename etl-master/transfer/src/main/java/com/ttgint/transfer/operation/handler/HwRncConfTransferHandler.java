...
@Slf4j
public class HwRncConfTransferHandler extends TransferBaseHandler {

    private final List<String> nodes;

    public HwRncConfTransferHandler(ApplicationContext applicationContext,
    ...
    public void filterFiles() {
        log.info("* HwRncConfTransferHandler filterFiles and setFileInfos");
        super.filterFiles();
        getRemoteFiles().stream()
                .filter(e -> e.getFileName().startsWith("GExport_R"))
                .filter(e -> {
                    try {
