...
@Slf4j
public class HwBscCmTransferHandler extends TransferBaseHandler {

    private final List<String> nodes;

    public HwBscCmTransferHandler(ApplicationContext applicationContext,
    ...
    public void filterFiles() {
        log.info("* HwBscCmTransferHandler filterFiles and setFileInfos");
        super.filterFiles();
        getRemoteFiles().stream()
                .filter(e -> e.getFileName().startsWith("CMExport_B"))
                .filter(e -> {
                    try {
