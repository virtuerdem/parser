...
@Slf4j
public class HwNbConfTransferHandler extends TransferBaseHandler {

    private final List<String> nodes;

    public HwNbConfTransferHandler(ApplicationContext applicationContext,
    ...
    public void filterFiles() {
        log.info("* HwNbConfCmTransferHandler filterFiles and setFileInfos");
        super.filterFiles();
        getRemoteFiles().stream()
                .filter(e -> e.getFileName().startsWith("GExport_N")
                        && !e.getFileName().startsWith("GExport_NR"))
                .filter(e -> {
                    try {
