...
import com.ttgint.parserEngine.Northi.Vodafone.Parsers.HpCmTxtFileHandler;
 ...
 */
@ParserSystem(systemType = "HP", measType = "CM", operatorName = "VODAFONE")
public class ParserEngine_cm_Hp extends AbsParserEngine {

    public ParserEngine_cm_Hp(ParserSystem parserSystem) throws SQLException, InterruptedException, Exception {
        super(parserSystem);
    }
    ...
    public void prepareParser() throws ParserEngineException {
        ...
        for (File each : fileList) {
            if (!each.getName().endsWith(AbsParserEngine.integratedFileExtension)) {
                HpCmTxtFileHandler t1 = new HpCmTxtFileHandler(each, OperationSystemEnum.WINDOWS, ProgressTypeEnum.PRODUCT);
