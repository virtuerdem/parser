...
import com.ttgint.parserEngine.Northi.Vodafone.Parsers.EricssonESSRGGsnNewParserHandler;
 ...
 * @author TTGETERZI
 */
@ParserSystem(systemType = "ESSR_GGSN", measType = "PM", operatorName = "VODAFONE")
public class ParserEngine_pm_Ericsson_essrggsn extends AbsParserEngine {

    public ParserEngine_pm_Ericsson_essrggsn(ParserSystem parserSystem) throws SQLException, InterruptedException, Exception {
        super(parserSystem);
    }
    ...
    public void prepareParser() throws ParserEngineException {
        ...
        for (File eachFile : fileList) {
            if (eachFile.getName().endsWith(".xml")) {
                executor.execute(new EricssonESSRGGsnNewParserHandler(eachFile, OperationSystemEnum.WINDOWS, ProgressTypeEnum.PRODUCT));
            }
        }
        executor.shutdown();
