...
import com.ttgint.parserEngine.Northi.Vodafone.Parsers.TwampCmTxtFileHandler;
 ...
 */
@ParserSystem(systemType = "TWAMP", measType = "CM", operatorName = "VODAFONE")
public class ParserEngine_TwampCm extends AbsParserEngine {

    public ParserEngine_TwampCm(ParserSystem parserSystem) throws SQLException, InterruptedException, Exception {
        super(parserSystem);
    }
    ...
    public void prepareParser() throws ParserEngineException {
        ...
        for (File each : fileList) {
            if (each.getName().endsWith(".csv")) {
                TwampCmTxtFileHandler t1 = new TwampCmTxtFileHandler(each, OperationSystemEnum.WINDOWS, ProgressTypeEnum.PRODUCT);
