 ...
 */
public class TwampCmTxtFileHandler extends FileReaderHandler {
    ...
    public TwampCmTxtFileHandler(File currentFileProgress, OperationSystemEnum operationSystem, ProgressTypeEnum progType) {
    ...
    public void lineProgress(String line) {
        rowCount++;
        try {
            line = line.replace(";", AbsParserEngine.resultParameter);
            ...
            if (rowCount == 1) {//header
                ...
                fileContentDataDate = currentFileProgress.getName().split("\\-", 2)[1].replace(".csv", "").replace("-", "");
                fileHeaderNames = "DATA_DATE" + AbsParserEngine.resultParameter + line;
            } else if (rowCount > 1 && tableObject != null) { //values
                String record = fileContentDataDate + AbsParserEngine.resultParameter + line;
