 ...
 */
public class HW5GCmXmlSaxParser extends SaxParserHandler {
    ...
    public HW5GCmXmlSaxParser(File currentFileProgress, OperationSystemEnum operationSystem, ProgressTypeEnum progType,
    ...
    public void endElement(String uri, String localName, String qName) throws SAXException {
        switch (qName) {
            ...
            case "MOTree":
                ...
                nodeMap.put("node.key.neVersion", neVersion.substring(neVersion.indexOf("R"), neVersion.indexOf("C")));
        ...
        try {
            ...
            if (tableObject != null) {
                ...
                switch (tableObject.getTableType()) {
                    case "ENODEB-CELL":
                    case "GNODEB-CELL":
                        neId = networkIdGenerator(
                                Integer.valueOf(list.get("node.key.neRawID")),
                                Integer.valueOf(list.get(tableObject.getNetworkIdType())),
                                getClassTypeId(tableObject.getTableType()),
                                getVendorId(tableObject.getNeType()));

                        parentId = networkIdGenerator(
                                Integer.valueOf(list.get("node.key.neRawID")),
                                Integer.valueOf(list.get("node.key.neRawID")),
                                getClassTypeId(tableObject.getTableType().split("\\-")[0]),
                                getVendorId(tableObject.getSystemType()));
                    ...
                    case "GNODEB":
                        ...
                        list.put("object.key.type", Integer.toString(ParserEngine_cm_Huawei.gNodeBClassTypeId));
                        list.put("object.key.neName", list.get(tableObject.getFunctionSubsetId()));
                        break;
                }
